...
var PermissionList = Class.create({
	...
	selectPermission: function(index) {
		...
		if (this.selectedIndex == -1) {
			...
			if (h3) {
				h3.update(language['wsip.acp.'+this.type+'.permissions.permissionsFor'].replace(/\{\$name\}/, permission.name));
			}
	...
	refreshSettings: function() {
		...
		if (settingsDiv) {
			...
			for (var i = 0; i < settingIndexes.length; i++) {
				...
				// create span
				var span = new Element('span').observe('mouseup', function(name) { $(name).focus(); }.bind(this, this.key+'Setting'+setting+'Allow')).insert(language['wsip.acp.'+this.type+'.permissions.'+setting]);
